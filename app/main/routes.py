...
from flask import flash, g, render_template, redirect
...
@login_required
def test():
    return render_template('test.html', title='test', data=current_app.config['SECRET_KEY'])
...
def search():
    ...
    # next_url = url_for('main.search', q=g.search_form.q.data, page=posts.next_num) \
    #     if posts.has_next else None

    # prev_url = url_for('main.search', q=g.search_form.q.data, page=posts.prev_num) \
    #     if posts.has_prev else None
